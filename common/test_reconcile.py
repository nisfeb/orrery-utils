...
class Participants(unittest.TestCase):
    def test_names_in_titles(self):
        ...
        self.assertEqual(n('Rose and Leo- Opti Sail'), (['Rose', 'Leo'], None))
        self.assertEqual(n('Milo Birthday'), (['Milo'], None))
        self.assertEqual(n("Seamus's birthday party"), (['Seamus'], None))
        self.assertEqual(n('Milo Fencing Lesson'), ([], 'Milo'))
    ...
    def test_people_out_of_titles(self):
        def act(bid, name, parts=()):
            ...
            return b
        state = {'bodies': [
            {'id': 'person/me', 'kind': 'person', 'name': 'owner', 'aliases': ['I'], 'attrs': {}, 'created': '2026-01-01T00:00:00Z'},
            {'id': 'person/leo', 'kind': 'person', 'name': 'Leo Baker', 'aliases': [], 'attrs': {}, 'created': '2026-01-01T00:00:00Z'},
            act('activity/adelaide-ballet-tap', 'Adelaide- Ballet/Tap', ['person/me']),
            act('activity/rose-and-leo-opti-sail', 'Rose and Leo- Opti Sail'),
            act('activity/milo-fencing-lesson', 'Milo Fencing Lesson'),
            act('situation/milo-birthday', 'Milo Birthday'),
            act('activity/nutcracker-rehearsal', 'Nutcracker rehearsal'),
            act('situation/owner-brave', 'the owner Brave Together'),
        ]}
        plan = reconcile.plan_participants(state)
        self.assertEqual([c['id'] for c in plan['creates']], ['person/adelaide', 'person/milo', 'person/rose'])
        rows = {(r['subject'], r['value']['ref']) for r in plan['rows']}
        self.assertEqual(rows, {('activity/adelaide-ballet-tap', 'person/adelaide'),
                                ('activity/rose-and-leo-opti-sail', 'person/rose'),
                                ('activity/rose-and-leo-opti-sail', 'person/leo'),
                                ('activity/milo-fencing-lesson', 'person/milo'),
                                ('situation/milo-birthday', 'person/milo'),
                                ('situation/owner-brave', 'person/me')})
