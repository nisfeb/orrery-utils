...
Two of those checks exist because a model reaches for them anyway. `kind/slug` is the literal placeholder in the prompt's own shape and matches the id pattern, so the kind is checked against a list rather than a regex. And an observation on a kind the schema lists attributes for keeps to that vocabulary, because the owner's `sensitive` policy shields attributes by name: a health fact written to an invented `soreness` or `concern` would be readable by every key, so it is dropped and the prompt says to use `health` or `income`. Those two are kept only when the key's schema lists them; a policy that marks them sensitive hides them from the key's schema, and then they are dropped like any other unlisted name. A kind the schema is silent on, such as `activity`,
...
`brief-prompt.md` beside it is the morning brief's system prompt, the one a client sends the frontier model each morning: what the brief is about (today, and the next two days only for what today must do for them), what it points out, and how short it is. A client that composes a brief sends this text word for word and holds it to the file with a test, the way the readers here hold `analyst-prompt.md`. Guide the brief by editing this file, not the client.

`generator-prompt.md` is the system prompt for orrery's own on-ship generator, the model that reads the whole state back and proposes what to do about it, rather than one message at a time: what it is given (the state, the recent decisions, the schema), what it should propose and what it should not, and the answer shape. `refine-prompt.md` is the system prompt for the ship's refine route, run when the owner types a note under a proposed action while approving it: what the note can change on the action itself, what a name the note gives that the ship lacks becomes, what goes in `extras` for a note that asks for more than the action, and when the model refuses. Both carry the owner's prose rules word for word, the same as `payloads.message.text` in the schema: no em dashes, no semicolons or colons joining independent clauses, simple direct sentences of varied length, and a sentence with more than one parenthetical thought split in two. Orrery holds all three cords, `generator-prompt.md`, `analyst-prompt.md` and `refine-prompt.md`, to these files byte for byte in `code/lib/orrery.hoon`, checked by `scripts/prompt-drift.py` in the orrery repo; a change here needs the same change there, or the gate fails.
